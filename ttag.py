...
class TileTagger:
    def __init__(self):
        ...
        self.tilemap = pyglet.sprite.Sprite(tileset_resource, x=0, y=0, batch=background_batch)
        self.panel = None
        self.label = None
        self.tile_x = 0
        self.tile_y = 0
...
class ShortTextInput:
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == pyglet.window.key.ENTER:
            self.tagger.property_index[self.tile_x, self.tile_y].add(self.label_item.text) 
            ...
            if self.text == ">":
                ...
                self.text = self.text + pyglet.window.key.symbol_string(symbol).lower()
                self.label_item.delete()
                self.update_label()
    ...
    def update_labels(self):
        ...
        for t in self.tags:
            ...
            label = pyglet.text.Label(text=t,
                                      font_name="Go Mono",
                                      x=self.x, y=self.y-int(bottom_y),
            ...
            self.panel_background = pyglet.shapes.Rectangle(x=self.x, y=self.y-int(bottom_y)+15, 
                                                            width=bottom_x,
                                                            height=bottom_y, 
                                                            color=(60, 40, 40),
